  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 pb-20 md:pb-0">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex space-x-8 mx-4">
               <button onClick={() => setView('LOG_TRIPS')} className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition ${view === 'LOG_TRIPS' ? 'border-blue-500 text-gray-900 dark:text-slate-50' : 'border-transparent text-gray-500 dark:text-slate-400 hover:border-gray-300 dark:hover:border-slate-600 hover:text-gray-700 dark:hover:text-slate-300'}`}>Log Trips</button>
               ...
               {canAccessAdminConsole && <button onClick={() => setView('ADMIN_CONSOLE')} className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition ${view === 'ADMIN_CONSOLE' ? 'border-blue-500 text-gray-900 dark:text-slate-50' : 'border-transparent text-gray-500 dark:text-slate-400 hover:border-gray-300 dark:hover:border-slate-600 hover:text-gray-700 dark:hover:text-slate-300'}`}>Admin Console</button>}
            </div>
            <div className="flex items-center justify-end flex-1 gap-2 sm:gap-4 ml-8">
              <div className="relative max-w-xs w-full" ref={searchRef}>
                <div className="relative">
                  ...
                  <input type="text" className="block w-full pl-10 pr-10 py-2 border border-slate-300 dark:border-slate-600 rounded-full leading-5 bg-slate-50 dark:bg-slate-700 placeholder-slate-400 dark:placeholder-slate-400 focus:outline-none focus:bg-white dark:focus:bg-slate-700 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition text-slate-900 dark:text-slate-50 text-center focus:text-left" placeholder="Search" value={searchQuery} onChange={handleSearch} onFocus={(e) => { setIsSearchOpen(true); e.target.placeholder = "Search logs & users..."; }} onBlur={(e) => { e.target.placeholder = "Search"; }} />
                    ...
                    {filteredUsers.length > 0 && (
                      <div>
                        ...
                        {filteredUsers.map(user => (
                          <button key={user.id} onClick={() => navigateToMessage(user.id)} className="w-full text-left px-4 py-3 hover:bg-blue-50 dark:hover:bg-slate-700 transition flex items-center space-x-3 group">
                            <div className="bg-slate-100 dark:bg-slate-700 p-2 rounded-full group-hover:bg-blue-100 dark:group-hover:bg-blue-900/50 transition"><UserIcon size={16} className="text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400" /></div>
                            <div><div className="text-sm font-medium text-slate-800 dark:text-slate-200">{user.firstName} {user.lastName}</div>{currentUser?.role === UserRole.ADMIN && <div className="text-xs text-slate-500 dark:text-slate-400">{user.phone}</div>}</div>
                          </button>
                        ))}
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        {filteredLogs.map(log => (
                          <div key={log.id} className="px-4 py-3 border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                            ...
                            <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 mb-1"><UserIcon size={10} /> {log.driverName}</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500 flex flex-col gap-0.5"><span className="flex items-center gap-1"><MapPin size={10} /> {log.routeType === RouteType.HOTEL_TO_SITE ? 'Hotel -> Site' : 'Site -> Hotel'}</span><span className="flex items-center gap-1"><Calendar size={10} /> {new Date(log.timestamp).toLocaleDateString()} at {new Date(log.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span></div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div className="text-right hidden sm:block flex-shrink-0"><div className="text-sm font-medium text-slate-900 dark:text-slate-50">{currentUser.firstName} {currentUser.lastName}</div><div className="text-xs text-slate-500 dark:text-slate-400">{currentUser.phone}</div></div>
      ...
      <main className="py-6">
        ...
        {view === 'MESSAGES' && <MessagingView data={data} currentUser={currentUser} refreshData={fetchData} initialSelectedUserId={targetUserId} onUpdateProfile={handleUpdateProfile} />}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-t border-slate-200 dark:border-slate-700 flex justify-around p-3 md:hidden z-50">
        <button onClick={() => setView('LOG_TRIPS')} className={`flex flex-col items-center space-y-1 transition ${view === 'LOG_TRIPS' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}><LayoutDashboard size={24} /><span className="text-[10px] font-medium">Log Trips</span></button>
        ...
        {canAccessAdminConsole && <button onClick={() => setView('ADMIN_CONSOLE')} className={`flex flex-col items-center space-y-1 transition ${view === 'ADMIN_CONSOLE' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}><Settings size={24} /><span className="text-[10px] font-medium">Admin</span></button>}
      </div>
    </div>
  );
};
