...
const getCoachRailUrl = (crId: string | undefined | null, day: string): string => {
    if (!crId) return '#';
    const dayMap: Record<string, number> = {
        'monday': 1, 'tuesday': 2, 'wednesday': 3, 'thursday': 4,
        'friday': 5, 'saturday': 6, 'sunday': 7
    };
    const suffix = dayMap[day.toLowerCase()] || 1;
    return `https://app.coachrail.com/reservations/${crId}-${suffix}`;
};
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ data, refreshData, currentUser, theme }) => {
  ...
  // Fix: changed assigned_worksite_ids to assignedWorksiteIds to match the expected parameter type.
                ...
                {detailedSite.items.length === 0 ? (<div className="py-20 text-center text-slate-400 italic">No scheduled trips assigned to this worksite today.</div>) : detailedSite.items.map(({ trip, lifecycle }) => (
                    <div key={trip.id} className={`p-4 rounded-2xl border transition-all ${lifecycle.state === 'COMPLETE' ? 'bg-green-50/30 border-green-100 dark:border-green-900/30' : lifecycle.state === 'UNCONFIRMED' ? 'bg-amber-50/30 border-amber-100 border-dashed dark:border-amber-900/30' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 shadow-sm'}`}>
                        <div className="flex justify-between items-start mb-3">
                            ...
                            <div className="flex flex-col items-end gap-1">
                                {trip.amCrId && (<a href={getCoachRailUrl(trip.amCrId, trip.dayOfWeek)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[9px] font-black text-slate-400 hover:text-blue-500 uppercase">AM CR: {trip.amCrId} <ExternalLink size={10}/></a>)}
                                {trip.pmCrId && (<a href={getCoachRailUrl(trip.pmCrId, trip.dayOfWeek)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[9px] font-black text-slate-400 hover:text-indigo-500 uppercase">PM CR: {trip.pmCrId} <ExternalLink size={10}/></a>)}
                            </div>
                        </div>
